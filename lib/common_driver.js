...
module.exports = class MyDriver extends Homey.Driver {
  ...
  async onPair(session) {
    ...
    session.setHandler('list_devices', async () => {
      ...
      // get device data for the entire account to prevent API rate limiting (FREQUENTLY_ACCESS)
      ...
      if (list && Array.isArray(list)) {
        ...
        for (const dev of list) {
          ...
          if (!sn || processedSns.has(sn)) continue;
          processedSns.add(sn);
          ...
          if (validTypes.includes(dev.deviceType)) {
            ...
            // prepare base device
            const device = {
              name: `${firstSite.name || dev.deviceType.toUpperCase()} ${dev.deviceSn}`,
              data: {
                id: dev.deviceSn,
              },
              capabilities,
              settings: {
                ...
                plantId: `${firstSite.plant_id || ''}`,
                plantName: `${firstSite.name || ''}`,
                nominalPower,
                maxChargePower,
                maxDischargePower,
              },
            ...
            if (this.id === 'battery2' && hasBat2) {
              const device2 = {
                name: `${firstSite.name || dev.deviceType.toUpperCase()} ${dev.deviceSn}_2`,
                data: {
