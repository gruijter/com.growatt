      ...
      for (const site of sites) {
        ...
        for (const dev of list) {
          ...
          if (validTypes.includes(dev.deviceType)) {
            let nominalPower = 3000;
            let maxChargePower = 3000;
            let maxDischargePower = 3000;
            try {
              const devInfo = await growatt.getDeviceInfo({ deviceSn: dev.deviceSn, deviceType: dev.deviceType });
              if (devInfo) {
                const typeData = devInfo[dev.deviceType] || [];
                const dInfo = typeData.find((d) => (d.serialNum || d.deviceSn || d.inverterId || d.mixSn || d.spaSn || d.hpsSn || d.tlxSn || d.maxSn || d.pcsSn) === dev.deviceSn);
                if (dInfo) {
                  const p = Number(dInfo.pmax || dInfo.nominalPower || 0);
                  const c = Number(dInfo.maxChargePower || dInfo.chargePower || p);
                  const d = Number(dInfo.maxDischargePower || dInfo.dischargePower || p);
                  if (p) nominalPower = p;
                  if (c) maxChargePower = c;
                  if (d) maxDischargePower = d;
                }
              }
            } catch (e) {
              this.error('Failed to fetch device info for power limits', e.message || e);
            }
