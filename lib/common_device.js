...
module.exports = class MyDevice extends Homey.Device {
  ...
  async onInit() {
    try {
      ...
      // Auto-populate nominalPower / maxChargePower / maxDischargePower if they are at the default 3000
      const set = this.getSettings();
      if (set.nominalPower === 3000 || set.maxChargePower === 3000 || set.maxDischargePower === 3000) {
        this.growatt.getDeviceInfo({ deviceSn: this.deviceSn, deviceType: this.deviceType })
          .then((devInfo) => {
            if (devInfo && devInfo.pmax) {
              const pmax = Number(devInfo.pmax);
              if (set.nominalPower === 3000) this.setSetting('nominalPower', pmax);
              if (set.maxChargePower === 3000) this.setSetting('maxChargePower', pmax);
              if (set.maxDischargePower === 3000) this.setSetting('maxDischargePower', pmax);
            }
          })
  ...
  async migrate() {
    try {
      ...
      // dynamically set capabilityOptions for battery target_power
      if (this.hasCapability('target_power') && this.driver.id === 'battery2') {
        const set = this.getSettings();
        const min = -(Number(set.maxChargePower || set.nominalPower || 3000));
        const max = Number(set.maxDischargePower || set.nominalPower || 3000);
  ...
  async onSettings({ oldSettings, newSettings, changedKeys }) {
    ...
    if (this.hasCapability('target_power') && (changedKeys.includes('maxChargePower') || changedKeys.includes('maxDischargePower'))) {
    ...
    try {
      ...
      for (const cap of capabilities) {
        if (growattMap.nonLastDataMap[cap]) {
          ...
          try {
            let method = growattMap.nonLastDataMap[cap];
            if (typeof method === 'function') method = method(this.deviceType, this.driver.id);
            ...
            if (data !== undefined && data !== null) {
              ...
              const value = capFunc(data, this.getSettings());
  ...
  async handleData(data) {
    ...
    for (const [cap, func] of Object.entries(capFuncs)) {
      try {
        this.setCapability(cap, func[0](data, this.getSettings())).catch((error) => this.error(error));
      } catch (err) {
