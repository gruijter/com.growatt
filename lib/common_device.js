...
module.exports = class MyDevice extends Homey.Device {
  ...
  async onInit() {
    try {
      ...
      // Auto-populate nominalPower if it's currently at the default 3000
      if (this.getSettings().nominalPower === 3000) {
        this.growatt.getDeviceInfo({ deviceSn: this.deviceSn, deviceType: this.deviceType })
          .then((devInfo) => {
            if (devInfo && devInfo.pmax) {
              this.setSetting('nominalPower', Number(devInfo.pmax));
            }
          })
          .catch((e) => this.error('Failed to fetch nominalPower', e.message || e));
      }
