  ...
  async handleFlowAction({ action, val }) {
    try {
      ...
      if (mapFunc) {
        ...
        this.log(`Executing flow action '${action}' to '${val}' via API (call: ${call}, mapped value: ${value})`);
        ...
        if (mapFunc) {
          ...
          if (!this.capabilityListeners[cap]) {
            ...
            this.registerCapabilityListener(cap, (val) => {
              const { call, value } = mapFunc(val, this.getSettings());
              this.log(`Setting capability '${cap}' to '${val}' via API (call: ${call}, mapped value: ${value})`);
