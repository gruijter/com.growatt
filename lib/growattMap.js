...
// Helper to calculate Solar AC output by proportionally distributing DC-AC conversion losses
const calcSolarAC = (pac, ppv, pbat) => {
  const ac = Number(pac ?? 0);
  const dcPv = Number(ppv ?? 0);
  const dcBat = Number(pbat ?? 0);
  if (dcPv === 0) return 0;
  if (dcBat === 0) return ac > 0 ? ac : dcPv; // fallback to DC if AC is missing
  const totalDc = dcPv + dcBat;
  return totalDc > 0 ? Math.round(ac * (dcPv / totalDc)) : 0;
};
...
const inverterMap = {
  ...
  // },
  inv: {
    measure_power: [(data) => Number(data.pac ?? data.ppv)], // AC Output from Solar
    'measure_power.dc_solar': [(data) => Number(data.ppv)],
    ...
    'dim.active_power': [(val) => Number(val?.value ?? val) / 100, (val) => ({ call: 'setActivePower', value: String(Math.round(100 * Number(val))) })], // max power 0 - 100 %
  },
  storage: {
    measure_power: [(data) => calcSolarAC(data.pac, data.ppv, data.pdischarge)], // AC Output from Solar
    'measure_power.dc_solar': [(data) => Number(data.ppv)],
    'measure_power.ac_inverter': [(data) => Number(data.pac)],
    meter_power: [(data) => Number(data.epvTotal)], // Total Yield kWh
    'meter_power.today': [(data) => Number(data.epvToday ?? ((data.epv1Today ?? 0) + (data.epv2Today ?? 0) + (data.epv3Today ?? 0) + (data.epv4Today ?? 0)))], // Today Yield kWh
    'dim.active_power': [(val) => Number(val?.value ?? val) / 100, (val) => ({ call: 'setActivePower', value: String(Math.round(100 * Number(val))) })], // max power 0 - 100 %
  },
  ...
  sph: {
    measure_power: [(data) => calcSolarAC(data.pac, data.ppv, data.pdischarge1)], // AC Output from Solar
    'measure_power.dc_solar': [(data) => Number(data.ppv)],
    'measure_power.ac_inverter': [(data) => Number(data.pac)],
    meter_power: [(data) => Number(data.epvTotal)], // Total Yield kWh
    'meter_power.today': [(data) => Number(data.epvToday ?? ((data.epv1Today ?? 0) + (data.epv2Today ?? 0) + (data.epv3Today ?? 0) + (data.epv4Today ?? 0)))], // Today Yield kWh
  ...
  min: {
    measure_power: [(data) => calcSolarAC(data.pac, data.ppv, Number(data.bdc1DischargePower ?? 0) + Number(data.bdc2DischargePower ?? 0))], // AC Output from Solar
    'measure_power.dc_solar': [(data) => Number(data.ppv)],
    'measure_power.ac_inverter': [(data) => Number(data.pac)],
    meter_power: [(data) => Number(data.epvTotal)], // Total Yield kWh
    'meter_power.today': [(data) => Number(data.epvToday ?? ((data.epv1Today ?? 0) + (data.epv2Today ?? 0) + (data.epv3Today ?? 0) + (data.epv4Today ?? 0)))], // Today Yield kWh
  ...
  noah: {
    ...
    // 'dim.active_power': [(val) => Number(val?.value ?? val) / 800, (val) => ({ call: 'setActivePower', value: String(Math.round(800 * Number(val))) })], // max power 0 - 100 % @ 800 W
  },
};
