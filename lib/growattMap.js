...
const setTargetPower = (val, settings) => {
  ...
  return { call: 'setActivePower', value: String(Math.max(0, Math.min(100, percentage))) };
};
...
const getBatTargetPower = (val, settings) => Math.round(-1 * (Number(val?.value ?? val) / 100) * Number(settings?.nominalPower || 3000));
const setBatTargetPower = (val, settings) => {
  const percentage = Math.round(-1 * (Number(val) / Number(settings?.nominalPower || 3000)) * 100);
  return { call: 'setChargeSetpoint', value: String(Math.max(-100, Math.min(100, percentage))) };
};
...
const batteryMap = {
  ...
  storage: {
    ...
    target_power: [getBatTargetPower, setBatTargetPower],
  },
...
const inverterMap = {
  ...
  inv: {
    ...
    target_power: [getTargetPower, setTargetPower],
  },
  storage: {
    measure_power: [(data) => calcSolarAC(data.pac, data.pcharge, data.pdischarge)], // Gross Solar Yield
    'measure_power.dc_solar': [(data) => Number(data.ppv)],
    meter_power: [(data) => Number(data.epvTotal)], // Total Yield kWh
    'meter_power.today': [(data) => Number(data.epvToday ?? ((data.epv1Today ?? 0) + (data.epv2Today ?? 0) + (data.epv3Today ?? 0) + (data.epv4Today ?? 0)))], // Today Yield kWh
  ...
  sph: {
    measure_power: [(data) => calcSolarAC(data.pac, data.pcharge1, data.pdischarge1)], // Gross Solar Yield
    'measure_power.dc_solar': [(data) => Number(data.ppv)],
    meter_power: [(data) => Number(data.epvTotal)], // Total Yield kWh
    'meter_power.today': [(data) => Number(data.epvToday ?? ((data.epv1Today ?? 0) + (data.epv2Today ?? 0) + (data.epv3Today ?? 0) + (data.epv4Today ?? 0)))], // Today Yield kWh
    'measure_power.ac_inverter': [(data) => Number(data.pac)],
  ...
  min: {
    measure_power: [(data) => calcSolarAC(
      data.pac,
  ...
  noah: {
    ...
    target_power: [(val) => Number(val?.value ?? val), (val) => ({ call: 'setActivePower', value: String(Math.round(Number(val))) })], // Absolute Watts
  },
};
...
const nonLastDataMap = {
  target_power: (deviceType, driverId) => (driverId === 'battery2' ? 'getChargeSetpoint' : 'getActivePower'),
